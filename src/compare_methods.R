...
# Load the calclators non-scaled and scaled
source("src/calculators.R")

# Create a value space taht covers all five variables evenly
d1f <- seq(0, 1, by = 0.1)
d2f <- seq(0, 1, by = 0.1)
d3f <- seq(0, 1, by = 0.1)
ltf <- seq(0, 1, by = 0.1)
...
# 1. elite_scaled = traditional elite aggregation stretched to range from 0 to 1 (interpertation of the value "proportion from minimum")
# 2. hh = weighted aritmentic mean of all indices (similar is used in habitat hectares approach, thus the  hh)
...
# Draw random "sites to be developped" and then another to compensate it and compute requiret ofset ratio
...
# elite requires on average larger compensation ratios than hh.
# If the random site is compensated with one that is in worse condition (compensation ration is higher than 1)
# elite requires in general smaller compensation ratios than hh.
